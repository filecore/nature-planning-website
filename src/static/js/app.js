(function () {
  ...
  const LAYERS = [
    ...
    { id: 'regional-recreation-areas', file: 'regional-recreation-areas.geojson', label: 'Regional plan recreation areas', color: '#4d7c0f', icon: '\u{1F9ED}', group: 'hiking' },
    { id: 'hiking-trails',    file: 'hiking-trails.geojson',    label: 'Hiking trails',                    color: '#c2410c', icon: '\u{1F6B6}', group: 'hiking' },
    ...
    { id: 'tree-species',     file: 'tree-species.geojson',     label: 'Tree species composition',         color: '#2d6a1f', icon: '\u{1F333}', group: 'natural-flora' },
  ...
  const GROUPS = {
    ...
    'natural-flora':   { label: 'Natural flora' },
    'wildlife':        { label: 'Wildlife' },
    'swimming-water':  { label: 'Swimming and water' },
    'quality':         { label: 'Environmental quality' },
    'alcohol':         { label: 'Alcohol' },
  };
  const GROUP_ORDER = ['hiking', 'natural-sites', 'geological', 'natural-flora', 'wildlife', 'swimming-water', 'quality', 'alcohol'];
  ...
  function initMap() {
    ...
    // Re-thicken line features (trails) on zoom - see lineWeightForZoom.
    map.on('zoomend', () => {
      const weight = lineWeightForZoom(map.getZoom());
      for (const entry of leafletLayers.values()) {
        if (!entry.polygonLayers) continue;
        for (const lyr of entry.polygonLayers) {
          if (lyr._kind === 'line') lyr.setStyle({ weight });
        }
      }
    });
  }
  ...
  // Line features (trails) at a flat weight are hard to hover precisely
  // at low zoom and nearly invisible at high zoom, since a fixed pixel
  // width covers a shrinking ground distance as you zoom in. Scale it up
  // with zoom instead, capped so it doesn't turn into a fat ribbon at max
  // zoom. Polygons keep their own fixed weight (an outline, not the thing
  // you need to hover).
  function lineWeightForZoom(zoom) {
    return Math.min(7, 2.5 + Math.max(0, zoom - FINLAND_ZOOM) * 0.6);
  }
  ...
  function buildLeafletLayer(feature, layer) {
    ...
    if (isCell) {
      ...
      const isLine = feature.geometry
        && (feature.geometry.type === 'LineString' || feature.geometry.type === 'MultiLineString');
      const geoJSONOpts = {
        style: {
          color: layer.color,
          weight: isLine ? lineWeightForZoom(map ? map.getZoom() : FINLAND_ZOOM) : 1.5,
          fillColor: layer.color,
          fillOpacity: 0.18,
        },
      };
